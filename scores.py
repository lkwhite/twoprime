...
Assumes signal has been loaded into a genomedata archive.
'''
...
# XXX: not quite right
SUM_SCALES = sum(WEIGHTS[i] for i in range(-FLANK_SIZE, FLANK_SIZE))

def scoreA(genome, chrom, pos, verbose = False):
    '''
    ...
    .. math::
        ...
        S_i = max{ 1 - (2 * n_i + 1) / 
                   (0.5 * |mean_l - std_l| + n_i + 0.5 * |mean_r - std_r| + 1)
    ...
    l_data = genome[chrom][pos-FLANK_SIZE:pos]
    r_data = genome[chrom][pos:pos+FLANK_SIZE]
...
def calc_flanks(genome, chrom, pos):
    '''
...
def scoreB(genome, chrom, pos, verbose = False):
    '''
    ...
    .. math::
        ...
        S_i = | ( n_i - 0.5 * sum(scaled_l_flank) / sum(scales) +
                              sum(scaled_r_flank) / sum(scales) ) | / n_i + 1
    ...
    scaled_l_flank, scaled_r_flank = calc_flanks(genome, chrom, pos)
...
def scoreC(genome, chrom, pos, verbose = False):
    '''
